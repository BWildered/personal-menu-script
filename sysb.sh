...
# Function to configure source directory
configure_source() {
    local current_source=$(read_ini "source_dir" "")
    local history=$(read_ini "source_history" "")
    local options=()
    
    # Add history items to options
    if [ ! -z "$history" ]; then
        IFS=':' read -ra ADDR <<< "$history"
        for i in "${ADDR[@]}"; do
            options+=("$i")
        done
    fi
    
    # Add option to enter custom directory
    options+=("Enter custom directory")
    options+=("Back to main menu")
    
    # Display menu
    display_menu "Select Source Directory" "${options[@]}"
    local choice=$?
    
    if [ $choice -lt $(( ${#options[@]} - 2 )) ]; then
        # Selected a history item
        write_ini "source_dir" "${options[$